...
moveapi = cdll.LoadLibrary("/home/d3gan/programs/psmoveapi/build/_psmove.so")
trackerapi = cdll.LoadLibrary("/home/d3gan/programs/psmoveapi/build/libpsmoveapi_tracker.so")
...
while True:
    ...
    if rospy.get_time() - last_button_time > .5:
        ...
        if button == 64:
            ...
            if human_mode:
                if pause:
                    move_msg.data = [-7.000000000000002, -3.333333333333332, 0.45013427734375, 0.03623247146606445,
                                     -0.06508475542068481, -0.024073563516139984, 0.06926485151052475, 0.0,
                                     -13.5040283203125, 69.99999999999994, 330.0, -0.10000000000000001, 0.0, 1, 0,
                                     human]
    ...
    if human == 1:
        move_msg.data = [-7.000000000000002, -3.333333333333332, 0.45013427734375, 0.03623247146606445,
                         -0.06508475542068481, -0.024073563516139984, 0.06926485151052475, 0.0, -13.5040283203125,
                         69.99999999999994, 330.0, -0.10000000000000001, 0.0, 1, 0, human]
    else:
        move_msg.data = position.tolist() + orientation.tolist() + [float(button)] + robot_position.tolist() + [
            robot_ori[0], robot_ori[1], gripper, reward, human]
    move_pub.publish(move_msg)
    # print move_msg
    time.sleep(.01)
